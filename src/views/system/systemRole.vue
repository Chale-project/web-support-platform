  ...
  <div style="background:#fff ;padding:25px;position: relative">
    ...
    >
      <span slot="roleType" slot-scope="roleType">
        <span v-if="roleType=='default'">默认</span>
        <span v-else>其他</span>
      </span>
      <span slot="roleStatus" slot-scope="roleStatus">
        <span v-if="roleStatus=='enabled'">启用</span>
        <span v-else>禁用</span>
      </span>
      <span slot="row_handles" slot-scope="roleStatus">
        <span v-if="roleStatus=='enabled'">禁用</span>
        <span v-else>启用</span>
      </span>
    </a-table>
    <!--分页-->
...
<script>
  ...
  import { BASEURL, METHOD_GET, METHOD_POST, METHOD_DELETE, METHOD_PUT } from '@/api/public';
  ...
  const queryValue = [
    { param: 'roleStatus', name: '状态', value: null },
    { param: 'roleType', name: '角色类型', value: null },
    { param: 'roleCode', name: '角色代码', value: null },
    { param: 'roleName', name: '角色名称', value: null }
  ];
  const queryData = [
    { param: 'id', name: '角色ID', required: true, value: null },
    { param: 'roleType', name: '角色类型', required: true, value: null },
    { param: 'roleCode', required: true, name: '角色代码', value: null },
    { param: 'roleName', required: true, name: '角色名称', value: null },
    { param: 'roleStatus', name: '角色状态', required: true, value: 'enabled'},
    { param: 'roleRemark', name: '备注', value: null },
    { param: 'appId', name: '应用ID', required: true, value: 'c1384aa20d4711e8b3d2fc4dd44915e7' },
    { param: 'roleBelongId', name: '角色归属ID', value: null }
  ]
  //数据
  const data = [{}];
  const query = [];
    ...
    methods:{
      ...
      handleAddOrEdit(data) {
        ...
        if(this.optionType == 'add') {
          // 新增
          this.addOrEditDatalist('basic/role/app/add', queryEditCondition, METHOD_POST)
        } else if (this.optionType == 'edit') {
          // 编辑
          this.addOrEditDatalist('basic/role/app/modify', queryEditCondition, METHOD_PUT)
        }
      },
      // 新增数据请求
      ...
      inquire(data) {
        ...
        if(this.appList.length > 0) {
          queryCondition["appId"] = this.appId
        }
        data.forEach(item => {
          queryCondition[item.param] = item.value
        })
        console.log(queryCondition)
        this.axios({
          url:  'basic/role/app/pageList',
          method: METHOD_POST,
          data: {
            currentPage:1,
            pageSize:10,
